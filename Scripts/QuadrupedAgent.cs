...
public class QuadrupedAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (Vector3.Dot(this.transform.up, Vector3.up) < minUpVectorDot || chest.GetComponent<TouchingGround>().touching || GetComplacency() > 0.5f)
        {
            ...
            for (int i = 0; i < limbs.Length; i++)
            {
                //Debug.Log(limbs[i].name);
                limbs[i].rotation = Quaternion.identity;
                
                float xRot = 0;

                if (limbs[i].name == "upper")
                {
                    xRot = -45;
                }
                if (limbs[i].name == "lower")
                {
                    xRot = 45;
                }
                limbs[i].Rotate(new Vector3(xRot, 0, 0));
            }
        }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        //limb positions vec3 * 8
        //limb rotation vec3 * 8
        //limb touchign ground bool * 8
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        if (chest.GetComponent<TouchingGround>().touching || Vector3.Dot(this.transform.up, Vector3.up) < minUpVectorDot)
        {
            //If fallen down or fell over
        ...
        float movementSmoothness = 1 - (float)System.Math.Tanh(velocityDelta/1.5f);
        ...
        float vttl1 = Mathf.Max((float)System.Math.Tanh(velocityTowardsTarget / 6), 0);
        ...
        reward = facingTarget * vttl1 * efficiencyRollingAverage * movementSmoothness * levelHorizon;
    ...
    private void DrawRedGreen(float reward)
    {
        ...
        {
            Color c = Color.Lerp(Color.red, Color.green, Mathf.InverseLerp(-1, 1, reward));
            rend.sharedMaterial.SetColor("_EmissionColor", c);
